...
- If raw SQL is needed (for example extensions, functions, triggers, or non-schema operations), generate the file with `cd apps/web && bun run db:generate --custom --name <name>` and then edit that generated file.
...
- `cd apps/web && bun run test:integration`: run integration tests against configured Postgres.
- `cd apps/web && bun run test:integration:local`: spin up local Docker Postgres, migrate, run integration tests, and tear down.
...
- `cd apps/web && bun run db:generate --name <name>`: generate a named Drizzle migration from schema changes.
- `cd apps/web && bun run db:generate --custom --name <name>`: generate a custom migration scaffold for raw SQL changes (do not create migration files manually).
- `cd apps/web && bun run db:migrate` / `cd apps/web && bun run db:push`: apply schema changes to Postgres.
...
- Unit tests use Bun: `cd apps/web && bun run test:unit`.
- Integration tests use Bun + Postgres: `cd apps/web && bun run test:integration` (run `cd apps/web && bun run db:migrate` first).
- Preferred local integration flow: `cd apps/web && bun run test:integration:local` (Docker required).
...
- Minimum validation for changes: `cd apps/web && bun run lint`, `cd apps/web && bun run build`, and relevant test commands.
- For DB changes, generate and commit a migration with `cd apps/web && bun run db:generate --name <name>`, then verify affected admin/storefront flows.
...
- Before committing, run formatting, linting, and relevant tests for your changes (`bun run format`, `cd apps/web && bun run lint`, and applicable `cd apps/web && bun run test:*` commands).
- Before committing, AI agents must run and pass all of the following commands in order: `bun run format:check`, `cd apps/web && bun run lint`, and `cd apps/web && bun run test:unit`.
