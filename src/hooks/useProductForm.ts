...
export function useProductForm({ categories, initialProduct, mode }: UseProductFormProps) {
  ...
  const hasChanges = (): boolean => {
    ...
    const idChanged = !isIdLocked && formData.id.trim() !== initialProduct.id;
    const nameChanged = (formData.name.trim() || null) !== initialProduct.name;
    const descriptionChanged = (formData.description.trim() || null) !== initialProduct.description;
  ...
  const getFormDataForSubmission = () => {
    ...
    if (mode === "add") {
      ...
      return {
        ...baseData,
        newId: !isIdLocked && formData.id.trim() !== initialProduct?.id 
          ? formData.id.trim() 
          : undefined,
      };
    }
