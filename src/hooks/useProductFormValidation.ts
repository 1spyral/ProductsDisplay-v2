...
export function useProductFormValidation({ 
  ...
  const getIdValidationMessage = useCallback((id: string): string | null => {
    ...
    if (isDuplicate && id !== initialId) return "This ID is already taken";
    return null;
  }, [isDuplicate, initialId, isValidIdFormat]);

  // Debounced duplicate checking
  const checkForDuplicate = useCallback(async (id: string) => {
    if (skipDuplicateCheck || id === initialId || !id.trim() || !isValidIdFormat(id)) {
  ...
  } as ProductValidationState & { checkForDuplicate: (id: string) => Promise<void> };
}
...
export function useProductIdValidation(
  ...
  const getIdValidationMessage = useCallback((id: string): string | null => {
    ...
    if (id.length > 255) return "ID too long (max 255 characters)";
    if (!isValidIdFormat(id)) return "Only letters, numbers, hyphens, and underscores allowed";
    if (isDuplicate && id !== options.initialId) return "This ID is already taken";
    return null;
  }, [isDuplicate, options.initialId, isValidIdFormat]);

  // Debounced duplicate checking function
  const checkForDuplicate = useCallback(async (id: string) => {
    if (options.skipDuplicateCheck || id === options.initialId || !id.trim() || !isValidIdFormat(id)) {
      setIsDuplicate(false);
      return;
    }
